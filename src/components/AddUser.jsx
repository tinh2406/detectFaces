...
export default function AddUser({ setAddUser }) {
    ...
    const handleCreateUser = async () => {
        ...
        setLoading(true)
        if (!openVerify) {
            axios.post(`${API_URL}:3000/users/addUser`, { phone, name, phoneOwner: user.phone })
                .then(res => {
            ...
            axios.post(`${API_URL}:3000/users/addUserExists`, { phone, name, phoneOwner: user.phone, verification })
            .then(res=>{
    ...
    return (
        ...
        <View style={{ marginBottom: 16 }}>
            ...
            <TextInput style={styles.textInput} value={name} onChangeText={text => setName(text)} placeholder="Name user" />
            ...
            {loading ? <ActivityIndicator size="large" color={'#ffffff'} /> : <Button color={(name && phone && phone !== user.phone && phone.trim().length === 10) ? "green" : "gray"} title="Add user" onPress={handleCreateUser} />}
